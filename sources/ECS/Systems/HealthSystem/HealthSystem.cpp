...
HealthSystem HealthSystem::operator()(Registry &registry, SparseArray<Component::Health> &healths, SparseArray<Component::Position> &positions)
{
    ...
    for (size_t i = 0; i < healths.size(); ++i)
    {
        ...
        if (health.has_value() && health.value().health <= 0) {
            if (pos.has_value() && draw.has_value() && rand() % 100 <= 35) {
                auto entity = registry.spawn_entity();
                Component::EntityClass entityClassTmp = Component::EntityClassFactory::CreateEntityClass(EntityClasses(rand() % 4 + 8));
                ...
                registry.add_component<Component::Group>(entity, Component::Group(0));
