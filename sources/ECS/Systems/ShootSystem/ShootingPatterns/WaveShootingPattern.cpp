...
void WaveShootingPattern::shoot(Registry &registry, Component::Shoot &shoot, Component::Position &pos, Component::Drawable &draw)
{
    ...
    if (shoot.canShoot == true && (shoot.m_keyMapState[sf::Keyboard::Space] == true || shoot.direction == -1) && elapsedTime >= shoot.shootDelay) {
        ...
        createProjectile(registry, draw, shoot.bulletSpriteName, Component::Position(x - projectileDraw.value().sprite.getTextureRect().width - 1, y - projectileDraw.value().sprite.getTextureRect().height - 10), Component::Position(0, 0), shoot.damage);
        createProjectile(registry, draw, shoot.bulletSpriteName, Component::Position(x - projectileDraw.value().sprite.getTextureRect().width - 1, y + projectileDraw.value().sprite.getTextureRect().height + 10), Component::Position(0, 1080), shoot.damage);
