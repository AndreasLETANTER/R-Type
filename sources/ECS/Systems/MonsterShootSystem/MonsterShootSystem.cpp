...
MonsterShootSystem MonsterShootSystem::operator()(Registry &registry, SparseArray<Component::MonsterShoot> &shoots, SparseArray<Component::Position> &positions, SparseArray<Component::Drawable> &drawable)
{
    for (size_t i = 0; i < shoots.size() && i < positions.size() && i < drawable.size(); i++) {
        ...
        if (shoot.has_value() && pos.has_value() && draw.has_value()) {
            ...
            if (shoot.value().canShoot == true && elapsedTime >= shoot.value().shootDelay) {
                ...
                registry.add_component<Component::Position>(projectile, Component::Position(x - (projectileDraw.value().sprite.getTextureRect().width + 50), y));
                registry.add_component<Component::Velocity>(projectile, Component::Velocity(0, 0));
                registry.add_component<Component::Projectile>(projectile, Component::Projectile(Component::Position(0, y), 10, shoot.value().damage));
